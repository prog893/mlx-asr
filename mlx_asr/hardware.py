...
batch grows (the fixed cost amortizes):
    ...
    M2 Ultra, 60s chunks: 0.152 GB/row at B=16 -> 0.114 at B=128
    M4,       60s chunks: 0.143 GB/row at B=16 -> 0.120 at B=48
...
function of *GPU cores*:
    ...
    M2 Ultra, 60 cores: 90% of peak throughput first reached at batch 64  (1.1 rows/core)
    M4,       10 cores: 90% of peak throughput first reached at batch 32  (3.2 rows/core)
...
def machine_info() -> dict:
    ...
    return {
        ...
        "ram_gb": round(int(ram) / 1e9) if ram.isdigit() else None,
