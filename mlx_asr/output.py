...
def build_cues(model, timed_chunks, chunk_offsets, gap_s=1.2, max_chars=28,
    ...
    **These values are deliberately NOT tuned to our references, because every
    timed reference available here comes from one production team and therefore
    encodes one editor's house style.** Two sweeps were run and both are recorded
    in RESULTS.md rather than applied:

    * n=1 (one file): suggested (1.2, 28), F1 35.4% -> 43.6%.
    * n=7 (seven files, `benchmarks/sweep_cues.py`): suggested (0.7, 32) instead,
      and showed the n=1 result only wins on the single file it was fitted to.
      End-to-end that was F1 36.6% -> 42.8%.
    ...
    *good segmentation*; it is evidence of agreement with one subtitle convention.
    All seven files share editors, line-wrap width and pause conventions, so
    "optimal gap_s" there may be a fact about that team rather than about
    Japanese subtitles. Fitting a shipped default to it would export their style
    to every user.
    ...
    * `gap_s` is the only knob that matters. Mean F1 moves 44.5 / 42.3 / 40.2 /
      36.2 / 35.9% across gap 0.6 / 0.7 / 0.8 / 1.0 / 1.2 on our references, so a
    ...
    * Lower gap_s buys F1 partly by emitting more cues (cues/ref 0.99 at gap 1.0,
    ...
    (Whisper reaches 52.6% F1 on the same files). Drift is the strong half and is
