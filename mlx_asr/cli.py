...
docs/PERFORMANCE.md), so "bigger batch is better" lands in the worst regime.

The flags are mostly NOT portable across the three engines, because the engines do
not share a long-form algorithm. Rather than accept a flag and drop it silently,
each path reports what it ignored: `_run_other_backend` lists the Voxtral-only
flags, and the Voxtral path reports `--language`, which is the one flag that
applies everywhere except here. `--chunk-seconds` is the single flag that crosses
over, and it means a different thing on each side (chunk length for Voxtral,
independent-window length for the chunked drivers, nothing at all for sequential
Whisper, whose 30s window is fixed by the model).
"""
...
def _run_other_backend(a, spec, log, t_start):
    ...
    # Every flag that does not apply here, so nothing the user passed is dropped
    # in silence. --delay-ms and --gain are included even though they have
    # non-None defaults: they are reported only when set away from the default,
    # since the default is not something the user asked for.
    ...
    if ignored:
        log(f"[{spec.backend}] ignoring Voxtral-only flags: {', '.join(ignored)}")
...
def main(argv=None):
    ...
    if a.language:
        log(f"[voxtral] ignoring --language: this model takes no language token "
            f"and detects the language itself")
    ...
    if a.fast:
        ...
        # (it is compute-bound, see docs/PERFORMANCE.md), so this trade is only
    ...
    if a.prompt and overlap_s > 0:
        ...
        log("[prompt] ignored: --prompt and --overlap-seconds conflict and "
            "together degrade output badly; overlap is kept (see RESULTS.md). "
    ...
    cues = build_cues(model, timed_all, list(offsets), time_map=time_map)
    total = time.perf_counter() - t_start
