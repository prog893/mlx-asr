...
of the same audio", which is a different and prior question.
    ...
    uv run python benchmarks/repeat_distribution.py \\
        --runs benchmarks/whisper_corpus_turbo-nocond-r*.json \\
        --baseline benchmarks/corpus_c30b32_kv8.json
"""
...
def load(path):
    ...
    rows = {round(float(r["duration_s"])): r
...
def main():
    ...
    if dropped:
        print(f"note: dropping {len(dropped)} incomplete run(s): "
              f"{', '.join(dropped)}\n")
    if len(full) < 2:
