...
showed the defaults split mid-phrase on ~59% of cues against human-authored
references, versus ~43% for Whisper, so the knobs are worth a sweep.
...
move a token's timestamp, so drift is invariant to these knobs by construction.
    ...
    uv run python benchmarks/sweep_cues.py --corpus corpus_timed --json out.json
"""
...
from evals.eval_timing import break_stats, parse_cues
...
def main():
    ...
    cur = next((r for r in rows if r["gap_s"] == 1.0 and r["max_chars"] == 32
                and r["max_dur_s"] == 7.0), None)
    if cur:
        print(f"current defaults (1.0/32/7.0): F1 {cur['break_f1']*100:.1f}% "
