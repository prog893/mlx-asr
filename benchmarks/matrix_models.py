...
The point is not accuracy (RESULTS.md has that on a proper corpus) but coverage:
...
the narration clip and nowhere else in this project (see eval_coverage.py).
    ...
    uv run --extra whisper --extra eval python benchmarks/matrix_models.py \\
        --audio 0816_180s.wav --reference ref_180s.txt --json matrix.json
"""
...
def main():
    ...
    scorer = None
    if ref:
        from evals.eval_cer import cer
        scorer = cer
