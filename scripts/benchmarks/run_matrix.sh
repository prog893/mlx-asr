...
#
#   sh scripts/benchmarks/run_matrix.sh <audio> <reference.srt> [model]
#
...
# scripts/benchmarks/compare_text_vs_srt.py. Emits one block per config so results paste
# straight into a findings doc under docs/benchmarks/.
#
...
OUT=${OUT:-bench_out}
# `metrics` is a package under scripts/, and this script is run from the repo root,
# so put scripts/ on the path rather than assuming the caller's cwd.
...
run() {
  ...
  $PY -m metrics.eval_cer "$REF" "$OUT/$label.txt" --verbose | tail -2
  $PY -m metrics.eval_cer_lenient "$REF" "$OUT/$label.txt" | tail -3
  echo "  -- srt-joined (content + cue layout) --"
  $PY -m metrics.eval_cer "$REF" "$OUT/$label.srt" | tail -1
}
