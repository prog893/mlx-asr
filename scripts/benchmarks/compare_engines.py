...
cancels. It is the method behind the corpus tables in docs/benchmarks/engines.md,
...
and a Whisper run compare directly. Files are matched by rounded duration rather
than by name, so that runs whose filenames were anonymized before sharing still
compare against local ones. Duration alone is not unique, so the scoring unit is
part of the key: see load_run.
...
unresolved rather than as a tie.
    ...
    uv run python scripts/benchmarks/compare_engines.py A.json B.json [--json out.json]
"""
...
def load_run(path: str):
    ...
    # `run_whisper.py` rewrites its JSON after every file, so an in-progress run on
    # disk is indistinguishable from a finished one except for this flag, and its
    # aggregate is a plausible-looking number over a subset. Refuse rather than
    # compare against it. Absent flag means a run from before it was added.
    if d.get("complete") is False:
        print(f"  WARNING: {path} is an INCOMPLETE run "
              f"({d.get('files_scored')} of {d.get('files_expected')} files); "
              f"its aggregate is a partial mean", file=sys.stderr)
    by_dur = {}
    for r in d.get("results", []):
        if "error" in r or "duration_s" not in r:
            continue
        # Duration alone is not unique: the corpus contains one recording dubbed
        # into two languages, identical in length to the sample. Keying on
        # duration alone silently dropped one of them. The unit disambiguates,
        # since a dub differs in script, and it is available on both sides.
        key = (round(float(r["duration_s"])), r.get("unit"))
        if key in by_dur:
            print(f"  WARNING: two files share {key} in {path}; the second is "
                  f"ignored, so this comparison is incomplete", file=sys.stderr)
            continue
        by_dur[key] = r
...
def paired(a_rows, b_rows, unit):
    ...
    ref, ea, eb, names = [], [], [], []
    for key in sorted(set(a_rows) & set(b_rows), key=lambda k: (k[0], str(k[1]))):
        ra, rb = a_rows[key], b_rows[key]
        ...
        if ra["ref_chars"] != rb["ref_chars"]:
            print(f"  WARNING: {key[0]}s file has {ra['ref_chars']} vs "
        ...
        names.append(ra.get("file", f"{key[0]}s"))
...
def main():
    ...
    if not shared:
        ...
        return 2
    only_a = sorted(set(a_rows) - shared, key=lambda k: (k[0], str(k[1])))
    only_b = sorted(set(b_rows) - shared, key=lambda k: (k[0], str(k[1])))
    if only_a or only_b:
