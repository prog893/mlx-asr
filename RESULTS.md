...
> two of this file's conclusions.** Everything below the "multi-file corpus"
> section is single-clip work: still correct as measured, but the rankings it
> reports do not generalize. Read the corpus section first.
...
`benchmarks/run_corpus.py`.

### The references deliberately omit audio, so plain CER is unusable

These transcripts were edited for storytelling: off-topic team chatter,
...
text the reference lacks. Plain CER reads 130-152% on the Japanese files as a
result, with thousands of "insertions" that are not errors.

`evals/eval_coverage.py` treats the reference as a subsequence to locate rather than the
...
text is excused as omitted-from-reference audio. `benchmarks/sweep_min_cut.py`
...
Full corpus sweep (`benchmarks/sweep_gain.py`, 30s/b32/kv8, coverage metric):
...
|---|---|---|---|
| M4, nvfp4 | 17.59% | 26.65% | 7.0x |
| M2 Ultra, 4bit | 18.21% | 27.75% | 21.4x |
...
## Whisper baseline on the same corpus (2026-07-29, M2 Ultra)

The obvious question the corpus makes answerable: how does this compare to
Whisper? Every Whisper size was run through `benchmarks/run_whisper.py`, which
scores with the *same* `evals/eval_coverage.py` functions on the *same* cached 16kHz
...
Paired per-file bootstrap (`benchmarks/compare_engines.py`, 20k resamples,
...
(`benchmarks/repeat_distribution.py`):
...
runs on three files (rec-04, rec-05, rec-06) and loses all 6 on three others
(rec-01, rec-03, rec-07); only rec-02 flips. So the aggregate is decided by which
files dominate the length weighting, not by sampling luck. On rec-03 Whisper
...
`benchmarks/sweep_min_cut.py` across engines says it is not:
...
would make it by far the worst model tested. That number was almost entirely an
...
Its numbers come from `benchmarks/run_kotoba_hf.py` on torch/MPS, so its accuracy
...
|---|---|---|---|
| **voxtral (M2 Ultra, 60s b16 kv8)** | - | **7.28%** | 21.2x |
...
Reproduce with `benchmarks/run_whisper_sweep.sh`, `benchmarks/run_kotoba_hf.py`,
then `summarize_engines.py`, `compare_engines.py` and `repeat_distribution.py`.

## Final corpus: 20 files, 7.95h (2026-08-04, M2 Ultra)
...
local recordings from the same production drives, and 5 YouTube uploads with
...
|---|---|---|---|
| local drives | 15 | 7 | 8 (Russian or English audio, Japanese transcript) |
| YouTube | 10 | 5 | 5 (English narration with translated Japanese subs) |

More than half failed. One file (`240421_007_Tr1`) has a Japanese transcript over
Russian crew chatter, and Voxtral scored it 199.51% coverage CER, which is the
metric correctly flagging a broken pair rather than a model failure. Two YouTube
uploads turned out to be the same video dubbed in two languages, and yt-dlp
serves the *default* track, so pinning `bestaudio[language^=ja]` is required or
you silently benchmark the dub.
...
Every metric above scores text and discards timestamps. `evals/eval_timing.py` scores
...
| | Voxtral (n=7) | whisper turbo-nocond (n=7) |
|---|---|---|
| median timing error | **258 ms** | 297 ms |
| median p95 error | **829 ms** | 1643 ms |
| worst drift slope | **37.1 ms/min** | 170.2 ms/min |
| break F1 | 42.8% | **52.6%** |
| mid-phrase splits | 57.1% | **43.0%** |

Voxtral's break figures are *after* the cue retune below; before it they were
36.6% F1 and 59.3% mid-phrase. Whisper's column includes one row the metric flags
as untrustworthy (9.8% anchor coverage, degenerate timestamps); dropping it lifts
Whisper's F1 to 58.3%, so these numbers are the conservative reading for this
project rather than the flattering one.

**They split cleanly: Voxtral holds timing, Whisper cuts cues.** Voxtral's worst
drift implies a few hundred ms across an hour. Whisper's 170.2 ms/min was measured
on a 9.3-minute file, i.e. about 1.6 seconds of accumulated drift by the end of it,
which is the failure that makes a subtitle file unusable and is invisible to CER.
In the other direction Whisper's cue boundaries match human editing far better.
...
One Whisper row was excluded by the metric's own rule. On the English file it
looped, repeating one line with degenerate timestamps (identical start and end,
and in one cue an end *before* its start), giving 18.1% anchor coverage. Below
50% coverage the timing estimate is untrustworthy, so it is dropped rather than
reported with its flattering 140ms median. Voxtral handled the same file at 94.5%
coverage.
...
With 7 timed references (`benchmarks/sweep_cues.py`, re-cueing a cached token
...
|---|---|---|---|
| 0.6 | 44.5% | 55.7% | 1.36 |
| **0.7 (new default)** | **42.3%** | **56.0%** | **1.25** |
| 0.8 | 40.2% | 56.5% | 1.11 |
| 1.0 | 36.2% | 59.1% | 0.99 |
| 1.2 (old default, with chars 28) | 37.0% | 58.1% | 1.01 |

Per file, against the shipped `1.2/28`:

| file | 1.2/28 (old) | 0.7/32 (new) |
|---|---|---|
| 0816_16k (**the file 1.2/28 was tuned on**) | **46.1%** | 35.9% |
| 2yQ7uxd7MVo | 38.2% | **49.2%** |
| EBDwjj8czIw | 28.6% | **36.5%** |
| UUVawCOfMGw | 32.4% | **41.2%** |
| fn_vD4wB6lM | 33.3% | **41.2%** |
| moYDTCX0GO8 | 41.9% | **48.8%** |
| vajAUtAbY3M | 38.8% | **43.6%** |

**The one file where the old default wins is the one it was fitted to.** It loses
on all six held-out files. That is overfitting visible in a single table, and it is
why the n=1 caveat at the top of this file exists.

Verified end-to-end by re-decoding all 7 files with the new default rather than
trusting the cached-token sweep: F1 36.6% -> 42.8%, mid-phrase 59.3% -> 57.1%,
median drift 248 -> 258 ms (`benchmarks/timing_engines_newcues.json`). Drift is
unchanged to within noise, as expected: regrouping cues cannot move a token's
timestamp.

Why 0.7 and not the top-scoring 0.6: F1 keeps rising as the gap falls, but so does
cue count (1.11 at 0.8, 1.25 at 0.7, 1.36 at 0.6). Part of that late gain is recall
bought by emitting more boundaries, which F1 rewards and a reader does not. 0.7
takes most of the win at a 25% cue-count increase. That is a judgment call, not a
measurement.
...
fire first. Both are kept as safety valves rather than tuned knobs. Even at 42.8%,
cue placement still trails Whisper's 52.6%, so this is the weaker half of the
project's timing story.
...
Whisper checkpoint (2-4 decoder layers) will misbehave under a sequential driver
...
`benchmarks/matrix_models.py`, raw rows in `benchmarks/matrix_models_m4.json`.
...
`--without-pip` venv: 65 wheels, all checksums matching, both engines
...
## Competing Apple Silicon runners (2026-08-04, M2 Ultra)
...
for the GPU. `benchmarks/compete_*.json`.
...
`docs/PERFORMANCE.md`; all rows are reproducible via `benchmarks/`.
...
inflating both intervals. `benchmarks/compare_configs.py`.
...
| M4: 30s chunks, overlap 8s vs 0s | 1.88 | [+0.73, +3.23] | **overlap wins** |
...
| M4: default (60s) vs --fast (30s+ov8) | 0.02 | [-0.81, +1.00] | not resolvable |
...
`benchmarks/rescore_all.py` reports a moving-block bootstrap CI as a partial
...
Metric note: `evals/eval_cer.py` (plain CER) has never changed, so every row is
mutually comparable. `evals/eval_cer_lenient.py` was added later and applied to the
...
Produced by `benchmarks/rescore_all.py` from the saved hypotheses (no re-decoding
...
| 17 | nvfp4, 60s chunks, batch 16, **--kv-bits 8** | **7.44%** | 125s | 7.5x | best M4 accuracy AND faster than #14: quantized KV halves cache reads (decode 78.9 -> 72.8s). Effectively free; needed a fix (QuantizedKVCache returns tuples, needs mlx-lm's quantized SDPA wrapper) |
...
| 19b | nvfp4, `--fast` (30s/b32 + kv8) | 9.23% | 105s | **8.9x** | best M4 speed; kv8 recovers most of what the extra seams cost (9.06% at 8.3x without kv8) |

## M2 Ultra (128GB, 60 GPU cores), 4bit affine, delay 2400ms
...
| 26 | 60s chunks, b16, --kv-bits 8 | 7.25% | 43.6s | 21.4x | same conclusion as M4 #17: quantized KV is free (7.23 -> 7.25) and slightly faster |
| 27 | 30s chunks, b32, --kv-bits 8 | 9.11% | 29.8s | **31.3x** | fastest run on either machine |
| 28 | 60s chunks, b16, --compact-silence | 8.23% | 42.1s | 22.2x | on this model compaction *helps* (9.13 -> 8.23 at 30s), unlike on the M4's nvfp4 |
| 29 | 30s chunks, b32, --compact-silence | 8.59% | 31.3s | 29.8x | del stays flat (123 -> 109), vs 115 -> 338 on M4 nvfp4 |

The Ultra reaches the best accuracy *and* 21x by using long chunks, because the
whole clip fits in one batch. On the M4 that choice costs speed. Chunk length
drives accuracy; batch size drives throughput.

**Silence compaction is model-dependent**, which is the one result that does not
transfer: identical audio and code, 12% of frames removed, yet nvfp4 on the M4
...
## Weight precision, including unquantized fp16 (M2 Ultra)

The 16GB M4 cannot hold fp16, so this is the test the Ultra existed for: is
quantization costing accuracy at all? Config held fixed at 60s chunks, batch 16,
delay 2400ms, same prompt; only the weights change. `benchmarks/sweep_precision.py`.
...
context. `benchmarks/probes/probe_seam_errors.py` confirms the mechanism by
...
M2 Ultra, 30s chunks, batch 32, kv8 (`benchmarks/sweep_overlap.py`):
...
M4, 30s chunks, batch 32: 0s 9.23% -> 4s 7.97% (-1.26) -> 8s **7.42%** (-1.81).
...
Testing whether even longer chunks beat 60s (M2 Ultra, 4bit, overlap 0):
...
### Batch-valley reshaping (M4, nvfp4, ms/step)
...
### Chunk-seam context carry (M2 Ultra, 30s chunks, batch 32)
...
`benchmarks/probes/probe_encoder_batch.py` reimplements the 32-layer encoder
...
so the encoder is **compute-bound** and already running near the M4's fp16 peak
...
encoder share of wall clock (35-45% on the M4) is a hard floor on this hardware,
not an optimization opportunity.

## Decode throughput vs batch size (synthetic, no audio)

`benchmarks/sweep_batch.py`. ms/step is what sets wall clock; x realtime is
steps/s x batch x 0.08.

| batch | M4 ms/step | M4 x rt | M2 Ultra ms/step | M2 Ultra x rt |
|---|---|---|---|---|
| 1 | 22.4 | 3.6 | 10.8 | 7.4 |
...
B=1 on the M4 is 22.4ms = 2.5GB / 120GB/s, exactly the bandwidth floor, so
...
| config | M2 Ultra text | M2 Ultra srt | M4 text | M4 srt |
|---|---|---|---|---|
...
(`benchmarks/run_matrix.sh` writes all formats per run), but they agree, and the
...
`benchmarks/ab_prompt.py`, now domain-agnostic: pass `--terms` or a
...
`benchmarks/ab_prompt.py`. Only the last 31 tokens of the prompt survive
truncation, silently.

| variant | tokens | CER M4 (nvfp4) | CER M2 Ultra (4bit) |
|---|---|---|---|
...
The machines disagree in sign: every variant helped on the M4 and every one hurt
...
excuses genuine homophone errors. `evals/eval_cer_lenient.py` puts real orthographic
