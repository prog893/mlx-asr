...
Reproduce with `benchmarks/matrix_models.py`; the raw rows are in
[benchmarks/matrix_models_m4.json](../benchmarks/matrix_models_m4.json). How the
metrics work and why the sample sizes constrain what you can read off them:
[BENCHMARKS.md](BENCHMARKS.md).

Do not pick a model from that table. It is one clip in one language, and the
20-file corpus in [RESULTS.md](../RESULTS.md) puts `whisper-turbo` ahead of
...
encoder is compute-bound, and the same code reaches 22.8x on an M2 Ultra (see
docs/PERFORMANCE.md).

The `kotoba` alias is v2.0 and gains nothing from v2.2. Both hold the same ASR
weights, matching to within 1e-4 across all 538 tensors, and v2.2's own model card
describes it as v2.0 plus speaker diarization and punctuation. Those are separate
post-processing models (`diarizers`, `punctuators`), not retrained weights, and
both want torch.
...
The engines do not share a long-form algorithm, so most flags apply to exactly
one of them. Rather than accept a flag and quietly drop it, the CLI names what it
ignored:

```console
$ mlx-asr audio.wav --model kotoba --chunk-seconds 20 --max-batch 8 --vad
[mlx-chunked] window length 20s (default 10s; material-dependent, so sweep it on your own audio)
[mlx-chunked] ignoring Voxtral-only flags: --max-batch, --vad
```

| flag | `voxtral` | `whisper-*` | `kotoba` |
|---|---|---|---|
| `--language` | ignored, reported | **used** (guessing costs 25 points) | forced to `ja` |
| `--chunk-seconds` | chunk length | ignored (30s window is fixed by the model) | **window length, its biggest lever** |
| `--max-batch` | yes | ignored | ignored |
| `--delay-ms` | yes | ignored | ignored |
| `--kv-bits` / `--no-kv-quant` | yes | ignored | ignored |
| `--fast` | yes | ignored | ignored |
| `--overlap-seconds` | yes | ignored | ignored |
| `--prompt` | yes | ignored | ignored |
| `--vad` | yes | ignored | ignored |
| `--compact-silence` | yes | ignored | ignored |
| `--gain` / `--peak-dbfs` / `--rms-dbfs` | yes | ignored | ignored |

`--chunk-seconds` is the only flag that means different things on different
engines. On `whisper-*` it is refused rather than approximated, because that
driver's 30s window is set by the model's positional encoding and a flag that
...
keys (`self_attn` to `attn`, `fc1` to `mlx1`, and so on) and swaps one axis on the
