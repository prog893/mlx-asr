...
`peak GPU memory` is measured, not derived: `mx.get_peak_memory()` on an M2 Ultra, over
the 20-file corpus for the default of each size and over one 93-minute file for the
intermediate quantization rungs. It is the number that decides whether a model fits, and
it barely tracks the download. whisper `tiny` downloads 0.07GB and peaks at 3.98GB,
because the working set is the 30s mel window and decoder activations rather than the
weights, so the whole whisper ladder fits in 4 to 7GB.

Your own figure is printed on every run and written to `--stats-json`, since peak memory
depends on the machine, the audio length and the flags.
